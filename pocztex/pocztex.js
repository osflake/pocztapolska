jQuery(document).ready(function(){
  ...
  $("#import-modal").load("./pocztex-modal.html"); 
  ...
  //console.log("Language: "+myParcelLanguage);
  ...
  $("#sledzenie-przekieruj").on("click",function(){
    ...
    if (sprawdzPrzesylke(parcel.val()) ) {
      if(parcel.val().length >6){
        location.href = "./pocztex-tracking.html?numer="+parcel.val();
      }else{
  ...
  function get_token(){
    ...
    jQuery.ajax({
      url: "https://uss.poczta-polska.pl/login",
      data: dane,
     error: function(e) {
    ...
    if (sprawdzPrzesylke(parcel.val()) ) {
      if(parcel.val().length >6){
        var params = {
            ...
            //console.log(jQuery("input[name='language']:checked").val());
        jQuery.ajax({
              url: ((jQuery("input[name='uss_v']:checked").val() == "2.0")? "http://194.93.124.155:8743":"https://uss.poczta-polska.pl")+"/uss/v"+jQuery("input[name='uss_v']:checked").val()+"/tracking/checkmailex",
             // url: "https://uss.poczta-polska.pl/uss/v"+jQuery("input[name='uss_v']:checked").val()+"/tracking/checkmailex",
//              url: "https://uss.poczta-polska.pl/uss/v1.1/tracking/checkmailex?number="+parcel.val()+"&language="+(get_params('language')?get_params('language'):jQuery("input[name='language']:checked").val()),
              //url: "https://api.poczta-polska.pl/track/v1/shipments/"+parcel.val()+"?" + $.param(params),
                beforeSend: function(xhrObj){
                    // Request headers
                    xhrObj.setRequestHeader("api_key",(jQuery("input[name='uss_v']:checked").val() == "2.0")? "oqmqN5t9WEf9r6bAMJ55mcB3RiJ1947+q32XyTitITAdKy5Zr/0KhpVefa5o1c2kyhdRRv2Ob6Q7BvGOTzw9F9XJfdtrJ3UfukZiDZkR6kOi5cNZR2sYQA5W84Ucha5kpojloceZl3DuKK6Q/J/y2QT4k+2vLrekByCkkSZ8PU0=.ODEyRTA0QTlFNEQ3MzNGNzA0RTQ3MENGRUI4MkM5MEE3RDQzNUZGMTcyNkMyQjJERjRGNUIwNjE0Q0QzQjAzRA==.d4d31169405a4a5eb8e33f0043390587":"BiGwVG2XHvXY+kPwJVPA8gnKchOFsyy39Thkyb1wAiWcKLQ1ICyLiCrxj1+vVGC+kQk3k0b74qkmt5/qVIzo7lTfXhfgJ72Iyzz05wH2XZI6AgXVDciX7G2jLCdoOEM6XegPsMJChiouWS2RZuf3eOXpK5RPl8Sy4pWj+b07MLg=.Mjg0Q0NFNzM0RTBERTIwOTNFOUYxNkYxMUY1NDZGMTA0NDMwQUIyRjg4REUxMjk5NDAyMkQ0N0VCNDgwNTc1NA==.b24415d1b30a456cb8ba187b34cb6a86");
...
//                var additionalInfo = "";
//                (parcelInfo.additionalServices.length>0)? jQuery.each(parcelInfo.additionalServices,function(a,b){additionalInfo =+ "<li>"+b.name+"</li>";}):"";
    ...
    //                console.log(value.kod,zdarzenia_pogrubienie.indexOf(value.kod));
                    // var DateCreated = new Date(Date.parse()).format("MM/dd/yyyy");
    ...
    //              divParcelAction.append("<h3>Status przesyłki: <small>Wersja 1</small></h3>");
    //              divParcelAction.append("<div class='row p-2 bg-danger text-white'><div class='col-5 h5'>Zdarzenie</div><div class='col-3 h5'>Data i czas</div><div class='col-4 h5'>Jednostka</div></div>");
    //              jQuery.each( (zdarzenia), function( key, value ) {
  ...
  //			location.href = 'https://emonitoring.poczta-polska.pl/?numer='+parcel.val().toUpperCase();
    ...
    $("#sledzenie-curl").on("click",function(){
      var spin = $(this).children(".spinner-border");
      //console.log("Język w funkcji");
      //console.log(get_params('language'));
      
      console.log("Metoda backedowa cURL: "); 
      //get_token();
      ...
      if (sprawdzPrzesylke(parcel.val()) ) {
        if(parcel.val().length >6){
          ...
          jQuery.ajax({
                url: "https://kartelmedia.co.uk/pocztex/checkParcel.php?number="+parcel.val()+"&language="+(get_params('language')?get_params('language'):jQuery("input[name='language']:checked").val()),
                  //url: "https://api.poczta-polska.pl/track/v1/shipments/"+parcel.val()+"?" + $.param(params),
  //                 beforeSend: function(xhrObj){
  //                     // Request headers
  //                     xhrObj.setRequestHeader("api_key","BiGwVG2XHvXY+kPwJVPA8gnKchOFsyy39Thkyb1wAiWcKLQ1ICyLiCrxj1+vVGC+kQk3k0b74qkmt5/qVIzo7lTfXhfgJ72Iyzz05wH2XZI6AgXVDciX7G2jLCdoOEM6XegPsMJChiouWS2RZuf3eOXpK5RPl8Sy4pWj+b07MLg=.Mjg0Q0NFNzM0RTBERTIwOTNFOUYxNkYxMUY1NDZGMTA0NDMwQUIyRjg4REUxMjk5NDAyMkQ0N0VCNDgwNTc1NA==.b24415d1b30a456cb8ba187b34cb6a86");
  // //                      xhrObj.setRequestHeader("Access-Control-Allow-Origin","*");
  // //                      xhrObj.setRequestHeader('Access-Control-Allow-Origin', 'http://localhost');
  // //                      xhrObj.setRequestHeader('Access-Control-Allow-Credentials', 'true'); 
  //                       //xhrObj.addRequestHeader("Host:","*");
  ...
  //                 },
                  ...
                  // data:{
                  //   'number' : parcel,
                  //   'language': (get_params('language')?get_params('language'):'pl'),
                  // },
                  //contentType: "application/json",
                  crossDomain: true,
              })
              .done(function(data) {
                spin.addClass("d-none");
                console.log("Dane przesyłki z BACKENDU CURL:");
                console.log(JSON.parse(data));
                parcelInfo = JSON.parse(data).mailInfo;
                jQuery("#parcelNumber").removeClass("d-none");
                //jQuery('#eMonitoring').modal('show');
                // 00259007738539428532
                // artur 00559007730113934232 >30dni
    ...
    //          console.log(modal);
                ...
                divParcelInfo.empty();
                divParcelAction.empty('');
                switch (parcelInfo.mailStatus){
                  case -2:
                    modal.find('#parcelInfo').html("<div class='alert alert-danger' role='alert'><h2>UWAGA!</h2><h3>Przesyłka o numerze <u>"+parcelInfo.number+"</u> nie istnieje!</h3></div>");
                    break;
                  case -99:
                  modal.find('#parcelInfo').html("<div class='alert alert-danger justify-content-center text-center' role='alert'><h2 class='p-2'>UWAGA!</h2><h3>Błąd systemu! Kod 99</h3></div>");
                  break;
                case -1:
                  modal.find('#parcelInfo').html("<div class='alert alert-danger justify-content-center text-center' role='alert'><h2 class='p-2'>UWAGA!</h2><h4>Przesyłka o numerze <u>"+parcelInfo.number+"</u> została nadana pond 30 dni temu! </h4><h4>Wgląd w dane jest niedostępny.</h4></div>");
                  break;
                case 2:
                  modal.find('#parcelInfo').html("<div class='alert alert-danger justify-content-center text-center' role='alert'><h2 class='p-2'>UWAGA!</h2><h3>Przesyłka o numerze <u>"+parcelInfo.number+"</u> istnieje, ale w podanym okresie nie ma zdarzeń do wyświetlenia</h3></div>");
                  break;
                case 1:
                  modal.find('#parcelInfo').html("<div class='alert alert-warning justify-content-center text-center' role='alert'><h2 class='p-2'>UWAGA!</h2><h3>Istnieje więcej przesyłek o numerze <u>"+parcelInfo.number+"</u></h3></div>");
                    break;
                  default:
    //			  			divParcelInfo.append("<table class='table border table-striped'><tbody>");
                  divParcelInfo.append("<h5>Dane przesyłki:</h5>");
                  divParcelInfo.append("<div class='row p-2'><div class='col-md-5 text-md-end p-2'><strong>Rodzaj endpointu</strong></div><div class='col-md-7 text-warning p-2'>WYSZUKIWANIE cURL (backend) metoda checkmail</div></div>");
                  divParcelInfo.append("<div class='row p-2'><div class='col-md-5 text-md-end'><strong>Data nadania:</strong></div><div class='col-md-7'>"+(moment(parcelInfo.dispatchDate).format('YYYY-MM-DD hh:mm'))+"</div></div>");
                  divParcelInfo.append("<div class='row p-2'><div class='col-md-5 text-md-end'><strong>Usługa:</strong></div><div class='col-md-7'>"+parcelInfo.typeOfMailName+" ("+parcelInfo.deliveryMode+")</div></div>");
                  divParcelInfo.append("<div class='row p-2'><div class='col-md-5 text-md-end'><strong>Kraj nadania:</strong></div><div class='col-md-7'>"+parcelInfo.dispatchCountryName+" ("+parcelInfo.dispatchCountryCode+") ");
    //			  			divParcelInfo.append("<div class='row p-2'><div class='col-md-5 text-md-end'><strong>Kraj nadania:</strong></div><div class='col-md-7'>"+parcelInfo.krajNadania+" ("+parcelInfo.kodKrajuNadania+") "+((parcelInfo.kodKrajuNadania !== "PL")?+"<br/><a href='#' class='btn p-1 btn-outline-dark'>Wyszukaj w kraju nadania</a>":"")+"</div></div>");
                  divParcelInfo.append("<div class='row p-2'><div class='col-md-5 text-md-end'><strong>Kraj doręczenia:</strong></div><div class='col-md-7'>"+parcelInfo.recipientCountryName+" ("+parcelInfo.recipientCountryCode+") <br/><a href='#' class='btn p-1 btn-outline-dark'>Wyszukaj w kraju doręczenia</a></div></div>");
                  (parcelInfo.typeOfMailCode)? divParcelInfo.append("<div class='row p-2'><div class='col-md-5 text-md-end'><strong>Gabaryt:</strong></div><div class='col-md-7'>"+parcelInfo.typeOfMailCode+"</div></div>"):'';
                  (parcelInfo.weight)? divParcelInfo.append("<div class='row p-2'><div class='col-md-5 text-md-end'><strong>Masa:</strong></div><div class='col-md-7'>"+parcelInfo.weight+" kg</div></div>"):'';
                  (parcelInfo.dispatchPostOffice.name)? divParcelInfo.append("<div class='row p-2'><div class='col-md-5 text-md-end'><strong>Miejsce nadania:</strong></div><div class='col-md-7'>"+parcelInfo.dispatchPostOffice.name+" </div></div>"):'';
                  (parcelInfo.recipientPostOffice.name)? divParcelInfo.append("<div class='row p-2'><div class='col-md-5 text-md-end'><strong>Miejsce doręczenia:</strong></div><div class='col-md-7'>"+parcelInfo.recipientPostOffice.name+" </div></div>"):'';
...
//            console.log(kraje);
           // var k=JSON.parse(kraje);
            
//            kraje = $.parseJSON(kraje);
           // kraje = $.csv.toArray(csv_file_API);
           ...
           /*kraje = $.getJSON({
            ...
            console.log(kraje);
            /*
              var obb = jQuery.grep(kraje, function(z,y){
                return z.Kraj=="Rosja";
              });*/
...
var myModalEl = document.getElementById('zewnetrznewww');
//console.log(myModalEl);
  myModalEl.addEventListener('shown.bs.modal', function (event) {
    var b = event.relatedTarget;

    // Extract info from data-bs-* attributes
    var url = b.getAttribute('data-bs-src');
    var del = b.getAttribute('data-bs-delete').split(',');
    var clas = b.getAttribute('data-bs-class');

  
    var createIframe = function (src,d) {
      var self = this;
      //console.log(self);
      $('<iframe>', {
          src: src,
        ...
        }).appendTo("#zewnetrznewww .modal-body");
        
  //      $("#zewnetrznewww .modal-body style").append("<style>nav{display: none !important;}</style>");

    };
    $("#zewnetrznewww .modal-body").empty();
    createIframe(url,del);
  
  //  $("#zewnetrznewww .modal-body").html('<iframe id="frameWWW" width="100%" height="100%" frameborder="0" scrolling="yes" allowtransparency="true" src="'+url+'"></iframe>');
    //$("#frameWWW").contents().find(del).remove();
  });
...
function generateState(st){ 
    ...
    var states=[
      {code:"NA",  icon:"fa-2x fa-box-open"},
      {code:"PRZ",  icon:"fa-2x fa-truck-loading"},
      {code:"TRA",  icon:"fa-2x fa-truck"},
      {code:"AUT",  icon:"fa-2x fa-map-marker-alt"},
      {code:"DOR",  icon:"fa-2x fa-home"}
    ];
    ...
    jQuery.each(states, function( key, value ) {
      ...
      //console.log(st_code);
      //kod =+ '"'+value.code+'"';
      kod.push("<i class='p-1 m-1 "+((st_code.indexOf(value.code)>=0)?'fas text-success ':'fa text-secondary ')+value.icon+"' title='"+value.name+"'></i>");
      //console.log("<i class='"+((st_code.indexOf(value.code)>=0)?'fas ':'fa ')+value.icon+"></i>");
      //console.log(((st_code.indexOf(value.code)>=0)?'s':'r'));
//      kod =+ (( (states).indexOf(value.code)>=0)?'=s=':'_r_');
    });
    kod.push("</div>");
    console.log(kod);
    return kod.join(" ");
