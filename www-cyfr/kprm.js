...
jQuery(".prev-step").on("click", function(){
    var param = "#nav-zloz-formularz-"+$(this).attr("aria-controls");
    $("#formularz-skladanie").find(".active").prop("disabled","disabled");
    $("#formularz-skladanie").find(".active").toggleClass("d-none d-sm-block");
    $(param).trigger("click");
});
...
jQuery(".kopiujDane").on("click",function(x,y){
  var n = jQuery(this).data("form");
  jQuery("form#adres_00").find("input").each(function(a,b){
    jQuery("#"+b.id+"_"+n).val(jQuery("#"+b.id).val());
...
jQuery(".podsumowanie").on("click",function(){
  console.log("OK");
  jQuery("#formularz-summary").empty().append("<div class='d-grid h3 gap-2 border p-2 bg-danger text-white mb-3'>Podsumowanie</div>");
  jQuery(".formularz").each(function(a,b){
    //console.log(b);
    var t = jQuery(b).children().not(".d-none").each(function(x,y){
      //var val = jQuery(y).find("input,textarea,select option:selected").val();
      console.log(y);
      //console.log(jQuery(this).hasClass("h4"));
      if(jQuery(this).hasClass("h4")){
        jQuery("#formularz-summary").append('<div class="d-block text-danger p-2 border-bottom">'+jQuery(this).text()+'</div>');
        console.log("TAK");
      }
      jQuery(y).find("input:not(:checkbox),textarea,select").each(function(s,m){
        console.log(s,m.id);
        if (((m.id)!== "") && (jQuery("#"+m.id).parent().parent().not(".d-none").length == 1) ){
          if(!jQuery(this).hasClass("file")){
            jQuery("#formularz-summary").append('<div class="d-flex mt-2">'+
        ...
        if(jQuery(this).hasClass("file")){
          console.log(this);
          jQuery("#formularz-summary").append(jQuery(".file-preview"));
        }
      });

//      console.log(idd,jQuery('label[for="'+idd+'"]').text());
/*      if(typeof(idd) === "undefined"){
        //jQuery("#formularz-summary").append(y);
      }else{
        jQuery("#formularz-summary").append('<div class="d-flex mt-2">'+
...
//    jQuery("#formularz-summary").append(b);    
  });
  jQuery("#formularz-summary > .navig").remove();
/*  jQuery("#formularz-summary").find(".form-control").each(function(a,b){
    console.log(a,b);
...
jQuery(".wyslijZgloszenie").on("click", function(){
  jQuery("#formularz-summary").empty().append('<div class="alert alert-success" role="alert">Dziękujemy! Twoje zgłoszenie zostało poprawnie złożone pod numerem 12397681238/2021</div>');
  
  //jQuery("#zloz-formularz-step-6").
});
